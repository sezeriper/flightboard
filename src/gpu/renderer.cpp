  ...
  const FPSCamera& camera)
{
  ...
  const auto group = registry.group<
    component::Position,
  ...
  {
    if (boundIndexBuffer != indexBuffer.value) gpu::bindIndexBuffer(context, indexBuffer.value);
    if (boundVertexBuffer != vertexBuffer.value) gpu::bindVertexBuffer(context, vertexBuffer.value);
    if (boundTexture != texture.value) gpu::bindSampler(context, texture.value);
